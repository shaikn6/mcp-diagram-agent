...
class DiagramGenerator:
    ...
    def generate(self, request: DiagramRequest) -> DiagramResponse:
        ...
        # Cap to max_elements at the spec level, before building elements, so
        # a diagram trimmed this way still shows connectivity: reserve a small
        # edge budget even when node count alone would consume the whole cap,
        # rather than always dropping every edge in favor of one more node.
        ...
        if len(raw_nodes) + len(raw_edges) > max_el:
            logger.warning(
                "Trimming %d nodes / %d edges to fit max_elements=%d",
                len(raw_nodes),
                len(raw_edges),
                max_el,
            )
            min_edge_budget = min(len(raw_edges), max(1, int(max_el) // 4)) if raw_edges else 0
            node_budget = max(1, int(max_el) - min_edge_budget)
            node_budget = min(node_budget, len(raw_nodes))
            kept_nodes = raw_nodes[:node_budget]
            kept_ids = {n.get("id") for n in kept_nodes}
            edge_budget = int(max_el) - len(kept_nodes)
            kept_edges = [
                e for e in raw_edges if e.get("from") in kept_ids and e.get("to") in kept_ids
            ][: max(edge_budget, 0)]
            spec = {**spec, "nodes": kept_nodes, "edges": kept_edges}
